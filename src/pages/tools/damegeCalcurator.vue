  ...
  <div class="container mx-auto relative">
    <section class="attacker-defender-setting-section flex flex-wrap">
      <section class="attacker border border-solid w-full p-4 lg:w-1/2 xl:w-1/2 m-2 shadow rounded">
        ...
        <SettingPokemonFrom @set-name="setNameAttacker" @set-move="setMoveAttacker" @set-item="setItemAttacker"/>
      </section>

      <section class="defender border border-solid w-full p-4 lg:w-1/2 xl:w-1/2 m-2 shadow rounded">
        <p class="text-s">防御側ポケモン</p>
        <SettingPokemonFrom :is-defender="true" @set-name="setNameDefender" @set-move="setMoveDefender" @set-item="setItemDefender"/>
      </section>
...
export default class DamageCalcurator extends Vue {
  ...
  createResult() {
    ...
    this.result = calc.calculate(8, this.attackerCal, this.defenderCal, this.attackerMove);
  }
}
</script>
